...
export async function createInstance(name: string, webhookUrl: string, proxyUri: string): Promise<{ device_id: string, name: string }> {
    const res = await fetch(`${API_BASE}/device`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, webhook_url: webhookUrl, proxy_uri: proxyUri }),
    })
